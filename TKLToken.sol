...
contract TKL is ERC20, ERC20Burnable, Pausable, AccessControl {
    ...
    function unlockGameBalance(address _to, uint256 _amount) public onlyRole(UNLOCKER_ROLE) returns(bool){
        ...
        (bool success, bytes memory data) = address(this).call(abi.encodeWithSelector(bytes4(keccak256(bytes('transfer(address,uint256)'))),_to, _amount));
        require(success && (data.length == 0 || abi.decode(data, (bool))),'ERC20::transfer: transfer failed');

        return true;
    }

    function unlockStakeBalance(address _to, uint256 _amount) public onlyRole(UNLOCKER_ROLE) returns(bool){
